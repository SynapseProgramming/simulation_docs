...
\begin{enumerate}
 ...
 \item {
       ...
       \begin{itemize}
        ...
        \item{
              As the mesh used in this example is in $mm$, the mesh volume would be in ${mm}^3$
              }
       \end{itemize}
...
\begin{enumerate}
 \item {
       ...
       (eg. s=$10^3$ for a mesh in $mm$. Thus, each $I$ value would have to be multiplied by $10^{-15}$)
 ...
 \item{
       As the generated mesh volume is in ${mm}^3$, we would have to multiply the mesh volume by $10^{-9}$ to scale it to $m^3$.
       }
\end{enumerate}
